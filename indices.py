...
def refresh(asof=None, offline=False, timeout=15, history_path=HISTORY_PATH, output_path=OUTPUT_PATH):
    ...
    with ThreadPoolExecutor(max_workers=3) as pool:
        for code, result, error in pool.map(one, SPECS):
            if result:
                ...
                old_points = previous.get('points') or []
                old_day = old_points[-1][0] if old_points else None
                new_day = result['points'][-1][0]
                if old_day and new_day < old_day:
                    regressions[code] = '官方源最新日%s早于已保存的%s；保留已保存日线' % (new_day, old_day)
                else:
                    saved[code] = result
            elif error:
                errors[code] = error
    ...
    for spec in SPECS:
        ...
        if history and (history.get('c'), history.get('n')) != (spec['c'], spec['n']):
            ...
            history = None
        status = ('cached' if offline or spec['c'] in errors or spec['c'] in regressions else 'available') if history else 'unavailable'
        record = make_record(spec, history, status, errors.get(spec['c']), asof)
        if spec['c'] in regressions:
            record['sourceRegression'] = regressions[spec['c']]
            record['note'] = regressions[spec['c']] + '。' + record['note']
    ...
    meta = dict(version=1, generatedAt=datetime.now(timezone.utc).isoformat(), requestedAsOf=asof,
                ...
                errors=errors, sourceRegressions=regressions,
                methodology='区间累计=期末收盘/周年日或此前最近交易日收盘−1；5年回撤为日收盘峰谷跌幅，波动为日对数收益样本标准差×√252。价格指数不含现金分红再投资。')
